...
: wxAuiToolBar(frame, id, wxDefaultPosition, wxDefaultSize, wxAUI_TB_DEFAULT_STYLE)
{
  ...
  //  EnableTool(ID_AUITOOLBARITEM8, false);
    ...
    frame->Connect(runItem->GetId(),wxEVT_COMMAND_MENU_SELECTED,wxCommandEventHandler(BuildToolbar::OnRun), NULL, this);
    frame->Connect(stopItem->GetId(),wxEVT_COMMAND_MENU_SELECTED,wxCommandEventHandler(BuildToolbar::OnStop), NULL, this);
}
...
void BuildToolbar::OnRun(wxCommandEvent& event)
{
    ...
    t->detach(); // detach wil ltell it to well detach from the main thread and close on its own when finished
